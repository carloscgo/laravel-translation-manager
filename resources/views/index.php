    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?php echo action('\CarlosCGO\TranslationManager\Controller@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?php echo action('\CarlosCGO\TranslationManager\Controller@getIndex') ?>';
                }
            });
...
<header class="navbar navbar-static-top navbar-inverse" id="top" role="banner">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\CarlosCGO\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                Translation Manager
            </a>
        </div>
    </div>
...
<div class="container-fluid">
    ...
    <p>
        <?php if(!isset($group)) : ?>
        <form class="form-import" method="POST" action="<?php echo action('\CarlosCGO\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
        ...
        <form class="form-find" method="POST" action="<?php echo action('\CarlosCGO\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
            <div class="form-group">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        ...
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\CarlosCGO\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?php echo $group ?>? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >Publish translations</button>
                <a href="<?= action('\CarlosCGO\TranslationManager\Controller@getIndex') ?>" class="btn btn-default">Back</a>
            </form>
        <?php endif; ?>
    </p>
    <form role="form" method="POST" action="<?php echo action('\CarlosCGO\TranslationManager\Controller@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
    ...
    <?php if($group): ?>
        <form action="<?php echo action('\CarlosCGO\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
        ...
        <table class="table">
            ...
            <?php foreach ($translations as $key => $translation): ?>
                <tr id="<?php echo $key ?>">
                    ...
                    <?php if ($deleteEnabled): ?>
                        <td>
                            <a href="<?php echo action('\CarlosCGO\TranslationManager\Controller@postDelete', [$group, $key]) ?>"
                               class="delete-key"
    ...
    <?php else: ?>
        <fieldset>
            ...
            <form  class="form-remove-locale" method="POST" role="form" action="<?php echo action('\CarlosCGO\TranslationManager\Controller@postRemoveLocale') ?>" data-confirm="Are you sure to remove this locale and all of data?">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            ...
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\CarlosCGO\TranslationManager\Controller@postAddLocale') ?>">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <div class="form-group">
                    <p>
        ...
        <fieldset>
            ...
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\CarlosCGO\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
